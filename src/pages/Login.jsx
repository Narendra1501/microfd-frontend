...
import { Mail, Lock, ArrowRight, Loader2, BookOpen, GraduationCap, Pencil, Sparkles, Laptop } from 'lucide-react';
...
const Login = () => {
    ...
    const handleLoginSubmit = async (e) => {
        ...
        if (!ptUnivRegex.test(formData.email)) {
    ...
    const handleResetSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            await api.post('/auth/reset-password', { email: resetEmail, password: formData.newPassword });
                    ...
                    <div className="relative z-10 w-full max-w-lg text-white">
                        ...
                        >
                            ...
                            <p className="text-lg text-slate-300 leading-relaxed opacity-90">
                                Join thousands of students shaping the future of education with MicroFeedback. Login to share your thoughts anonymously and daily.
                            </p>
                        </motion.div>
                    ...
                    >
                        ...
                        <div className="relative z-10">
                            ...
                            <h2 className="text-center text-3xl font-extrabold text-slate-900 tracking-tight">
                                {view === 'login' ? 'Welcome Back' : view === 'forgot' ? 'Identify Yourself' : 'Reset Password'}
                            </h2>
                            <p className="mt-3 text-center text-sm text-slate-500">
                                {view === 'login' ? (
                                    ...
                                    <button onClick={() => setView('login')} className="font-bold text-brand-600 hover:text-brand-500">Back to Login</button>
                                )}
                            </p>
                        </div>
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div className="group relative">
                                        <div className="flex justify-between items-center mb-1.5 ml-1">
                                            <label className="block text-sm font-semibold text-slate-700 transition-colors group-focus-within:text-brand-600">Password</label>
                                            <button type="button" onClick={() => setView('forgot')} className="text-xs font-bold text-brand-600 hover:text-brand-500 uppercase tracking-tighter">Forgot Password?</button>
                                        </div>
                                        <div className="relative">
                        ...
                        {view === 'forgot' && (
                            <form className="mt-10 space-y-6 relative z-10" onSubmit={handleForgotSubmit}>
                                <p className="text-xs text-slate-500 text-center mb-6">Enter your registered email to verify your identity.</p>
                                <div className="group relative">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input name="email" type="email" required className="input-field pl-11" placeholder="Enter your registered email" value={formData.email} onChange={handleChange} />
                                    </div>
                                </div>
                        ...
                        {view === 'reset' && (
                            <form className="mt-10 space-y-6 relative z-10" onSubmit={handleResetSubmit}>
                                <p className="text-xs text-brand-600 text-center font-bold mb-6">Verified: {resetEmail}</p>
